...
class TestHW3(unittest.TestCase):
    ...
    def test_d_put_key_value_operation(self):
        ...
        #for counter in range(self.keyCount):
        #    nodeIndex = counter % len(nodeIpList)
        #    response = requests.delete('http://localhost:' + nodeHostPortList[nodeIndex] + '/key-value-store/key' + str(counter), json={"causal-metadata": nextCausalMetadata}, timeout=TIMEOUT)
        #    responseInJson = response.json()
        #    print(responseInJson)
        #    self.assertEqual(response.status_code, 200)
        #    nextCausalMetadata = responseInJson["causal-metadata"]
        ...
        #nodeIndex = counter % len(nodeIpList)
        #response = requests.delete('http://localhost:' + nodeHostPortList[nodeIndex] + '/key-value-store/key' + str(800), json={"causal-metadata": nextCausalMetadata}, timeout=TIMEOUT)
        #responseInJson = response.json()
        #print(responseInJson)
    ...
    def test_i_possible_reshard(self):
        ...
        print(shard1, shard1KeyCount)
        print(shard2, shard2KeyCount)
        print(shard3, shard3KeyCount)
